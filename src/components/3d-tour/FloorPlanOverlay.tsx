...
interface FloorPlanOverlayProps {
  ...
  /** ETH-INT-001: Edit-mode callback. Receives percentage coords [0-100] of the click. */
...
export const FloorPlanOverlay: React.FC<FloorPlanOverlayProps> = ({
  ...
  return (
    <div className="absolute bottom-5 left-5 z-20 bg-neutral-50 dark:bg-neutral-950/90 backdrop-blur-md p-3 rounded-xl border border-red-600 dark:border-red-600/30 w-[240px] shadow-2xl">
      ...
      {isEditMode && (
        <p className="text-[9px] text-amber-500 mb-1.5 leading-tight">
          Click the floor plan to pin the active scene position.
        </p>
      )}

      <div
        className={`relative w-full aspect-video rounded-lg overflow-hidden border border-neutral-300 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800${isEditMode ? ' cursor-crosshair' : ''}`}
        ...
        {sceneWithCoords.map((scene) => {
          ...
          return (
            <button
              ...
              style={{ left: `${scene.fp_x}%`, top: `${scene.fp_y}%` }}
        ...
        {sceneWithCoords.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center">
            <p className="text-[10px] text-neutral-500 text-center px-4">
              {isEditMode ? 'Click to pin active scene position' : 'No room positions mapped yet'}
            </p>
          </div>
        )}
      </div>
