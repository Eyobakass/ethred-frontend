...
export const HotspotOverlay: React.FC<HotspotOverlayProps> = ({
  ...
  return (
    ...
    >
      <div className="w-full max-w-md bg-neutral-900 border border-gold-500/40 rounded-2xl p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-gold-400">📍 Author 3D Hotspot Pin</h3>
          <button
            onClick={onCancel}
        ...
        <div className="flex items-center gap-3 mb-4 p-3 rounded-xl bg-neutral-800 border border-neutral-700">
          <span className="text-xs text-neutral-400 font-mono">
            Pitch: <span className="text-gold-400">{pitch.toFixed(2)}°</span>
          </span>
          <span className="text-neutral-600">|</span>
          <span className="text-xs text-neutral-400 font-mono">
            Yaw: <span className="text-gold-400">{yaw.toFixed(2)}°</span>
          </span>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Pin type selector */}
          <div>
            <label className="block text-xs font-semibold text-neutral-300 mb-1.5">Pin Type</label>
            ...
            <div>
              ...
              {availableScenes.length === 0 ? (
                ...
                <select
                  ...
                  className="w-full bg-neutral-800 border border-neutral-700 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none focus:border-gold-500 transition"
                >
          ...
          {type === 'INFO' && (
            <div>
              ...
              <input
                ...
                maxLength={120}
                className="w-full bg-neutral-800 border border-neutral-700 rounded-xl px-3 py-2.5 text-xs text-white placeholder-neutral-500 focus:outline-none focus:border-gold-500 transition"
                required
              />
            </div>
          )}
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-neutral-800">
            ...
            <button
              type="submit"
              className="px-5 py-2 rounded-xl bg-gold-500 text-black text-xs font-bold hover:bg-gold-400 transition shadow-lg shadow-gold-500/20"
            >
              Save Pin
            </button>
          </div>
